...
# 세션 상태 초기화
if "run_after_add" not in st.session_state:
    st.session_state.run_after_add = False

if "run_after_delete" not in st.session_state:
    st.session_state.run_after_delete = False

# 채널 추가
st.subheader("➕ 벤치마킹 채널 추가 (여러 개 입력 가능)")
...
if st.button("채널 등록하기"):
    ...
    if new_data:
        ...
        st.session_state.run_after_add = True
        st.rerun()

# 등록 성공 메시지 표시
if st.session_state.run_after_add:
    st.success("✅ 채널이 성공적으로 추가되었습니다.")
    st.session_state.run_after_add = False

# 채널 목록 표시
st.subheader("📋 등록된 벤치마킹 채널 목록")
df = pd.read_excel(EXCEL_FILE)
if df.empty:
    ...
    for i, row in df.iterrows():
        ...
        if col4.button("삭제", key=f"del_{i}"):
            ...
            df.to_excel(EXCEL_FILE, index=False, engine="openpyxl")
            st.session_state.run_after_delete = True
            st.rerun()

# 삭제 성공 메시지 표시
if st.session_state.run_after_delete:
